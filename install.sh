#!/usr/bin/env bash
# install.sh — Install, uninstall, update, or check status of Forma skills
set -euo pipefail
...
get_skill_dirs() {
  local dirs=()
  for d in "$SKILLS_DIR"/*-in-forma; do
    [ -d "$d" ] && dirs+=("$d")
  done
  if [ ${#dirs[@]} -eq 0 ]; then
...
do_update() {
  ...
  for i in "${!PLATFORM_NAMES[@]}"; do
    ...
    for link in "$target_dir"/*-in-forma; do
      [ -L "$link" ] || continue
      local existing
      existing="$(readlink "$link")"
      case "$existing" in
...
usage() {
  ...
  echo "  uninstall   Remove Forma symlinks (leaves the clone intact)"
...
case "${1:-install}" in
  install)
    echo "Installing Forma skills..."
    do_install
    ;;
  uninstall)
    echo "Uninstalling Forma skills..."
    do_uninstall
    ;;
  update)
    do_update
